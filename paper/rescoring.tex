...
\large Once the decoder is strong, rescoring it barely helps}
...
only thing that varies is the rescorer. Sweeping 82M to 7.6B parameters moves word error
from \werNone\% to \werOpt\%, a range across which the 95\% intervals of all seven arms
overlap. After Holm correction for seven comparisons, only one arm separates from no
rescoring, and it is a closed model; no open rescorer does. Varying only the decoder's
acoustic weight on fixed candidate lists, the gain from rescoring falls from \gainWeak{}
points when the decoder errs at \fpWeakSweep\% to \gainStrong{} points at \werNone\%.
A cheaper design, asking the model to name the best candidate in a single pass rather
than scoring all 100 one by one, fails for every model we tried: they answer from where a
label sits in the list rather than from the sentence beside it. Every arm here is untuned and
text-only, so these are lower bounds on what the stage could deliver.
\end{abstract}
...
Our answers, measured on real neural data rather than simulation, are that almost nothing
in this stage still matters, and that the single-pass shortcut fails for a specific
reason. Both are lower bounds: every arm is untuned and text-only, so neither result
...
\item A scale study of the rescoring stage on real T15 recordings across a 93$\times$
  ...
  intervals and paired significance tests at each arm's tuned interpolation weight
...
\item A test that the result is not an artefact of how scores are combined: four
  interpolation schemes, a jointly fit multi-arm reranker, and MWER fine-tuning of the
  smallest rescorer all leave the ranking unchanged (\S\ref{sec:comb}).
\item A negative result on listwise typed selection. The measured cause is
...
efficiency comparison. Across a 93$\times$ parameter range word error moves three-tenths
of a point; after Holm correction only Jev clears significance.}
\label{tab:main}
\end{table}
...
scoring, triangles typed selection. The efficiency result is the flatness of the
per-candidate curve across two orders of magnitude of compute. Plotted points are the
exact Table~\ref{tab:main} values; the table rounds to one decimal.}
\label{fig:cost}
\end{figure}

Rescoring is barely worth doing once the decoder is this strong. The first pass alone scores
...
\input{tab_sig}
\caption{Paired bootstrap on the test split, each arm at its tuned $\alpha$.
...
open models, scale buys nothing measurable: OPT-6.7b against GPT-2 small is $-0.18$ at
$p=0.10$. The reading we favour is not that small models suffice, but that almost nothing in this
stage matters: it contributes a fraction of a point, so the choice of model inside it is a
...
\werNone\%. Past the published scale it grows again only because the scale itself
degrades the first pass. Over the range a practitioner would operate in, the value of
...
Where the published decoder sits, at \werNone\% error, three-tenths of a point remain.}
...
\section{Score combination is not the bottleneck}
\label{sec:comb}

\begin{table}[t]
\centering\small
\input{tab_combination}
...
ranking of arms is invariant to the rule, which is what \S\ref{sec:efficiency} needs.
...
(paired $p=\mwerMultiP$), keeping OPT-6.7b and Qwen2.5-7B and zeroing the rest. Even a
jointly fit discriminative combination of untuned scorers barely moves the needle.
...
only the closed Jev arm improves on it.
...
\item \textbf{One participant, one split, limited power.} T15, \texttt{val}, \nTest{} test
  sentences. Effects here are a few tenths of a point and bootstrap intervals span roughly
...
stage contributes a fraction of a word-error point. Across a 93$\times$ range of model
size word error moves three-tenths of a point, and no open-model difference clears
significance after Holm correction. The one arm that does is closed
(Jev~\cite{typesafe2026jev}); we read it as evidence that exploitable signal remains in
the task, not as a deployable result, and the reproducible finding is the negative one
across open models.
...
\gainStrong{} at \werNone\%. Four interpolation schemes, a jointly fit reranker and a
discriminatively fine-tuned GPT-2 small all land within noise, so this is not an artefact
